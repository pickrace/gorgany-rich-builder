...
function faqSoftUpdate() {
  ...
  document.getElementById('faqCountDesc').textContent = `${n} ${pluralPyt(n)} у коді`;
  faqItems.forEach((it, i) => {
    const w = document.getElementById(`faqWarn${i}`);
    if (!w) return;
...
function toggleFaqImport() {
  const p = document.getElementById('faqImportPanel');
  p.style.display = p.style.display === 'none' ? '' : 'none';
}

function faqImport(mode) {
  const text = document.getElementById('faqImportText').value;
  const parsed = text
    .split(/\n\s*\n+/)
    .map(b => b.trim())
    .filter(Boolean)
  ...
  document.getElementById('faqImportText').value = '';
  toggleFaqImport();
  faqRender();
}
...
function faqNameInput() {
  ...
  document.getElementById('faqIdHtml').textContent = idH;
  document.getElementById('faqIdJson').textContent = idJ;
  document.getElementById('faqOutIdHtml').textContent = idH;
  document.getElementById('faqOutIdJson').textContent = idJ;
}

function copyFaqId(kind) {
  const slug = faqSlugify(g('faqName'));
  if (!slug) return;
  const btn = document.getElementById(kind === 'html' ? 'faqIdHtmlBtn' : 'faqIdJsonBtn');
...
function toggleFaqOutput() {
  document.getElementById('faqOutputSection').classList.toggle('collapsed');
}

function faqGenerate() {
  const filled = faqFilled();
  const status = document.getElementById('faqStatusMsg');
  if (!filled.length) {
...
function setFaqOutputs(html, json) {
  const oH = document.getElementById('faqOutputHtml');
  const oJ = document.getElementById('faqOutputJson');
  oH.textContent = html;
  oH.classList.remove('output-empty');
  oJ.textContent = json;
  oJ.classList.remove('output-empty');
  const section = document.getElementById('faqOutputSection');
  section.classList.remove('collapsed');
  section.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}

function copyFaqCode(kind) {
  const el = document.getElementById(kind === 'html' ? 'faqOutputHtml' : 'faqOutputJson');
  const btn = document.getElementById(kind === 'html' ? 'faqCopyHtmlBtn' : 'faqCopyJsonBtn');
...
function renderFaqHistory() {
  const history = JSON.parse(localStorage.getItem('faqHistory') || '[]');
  const section = document.getElementById('faqHistorySection');
  const list = document.getElementById('faqHistoryList');
...
function loadFaqHistoryItem(i) {
  ...
  const e = history[i];
  if (e.slug) {
    document.getElementById('faqOutIdHtml').textContent = e.slug + '-faq-html';
    document.getElementById('faqOutIdJson').textContent = e.slug + '-faq-json';
  }
...
function copyFaqHistoryItem(i, kind) {
  const history = JSON.parse(localStorage.getItem('faqHistory') || '[]');
  if (!history[i]) return;
  const btn = document.getElementById((kind === 'html' ? 'faqHCopyH' : 'faqHCopyJ') + i);
